...
router.get('/Patients',requireLoginUser,(req,res)=>{
    ...
    .then(result=>{
        for (let i = 0; i < result.length; i++) {
            data.push({ id: result[i]._id, first_name: result[i].first_name, email: result[i].email, cin: result[i].cin, phone: result[i].phone, state: result[i].state, birhday: result.client[i].age })
        }
        res.json(data)
    })
    .then ((err)=>{
