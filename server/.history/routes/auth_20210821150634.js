...
router.post('/SignupPatient' ,(req,res)=>{
    //console.log(req.body.name)  --> testa3malha bish tjareb dima bil poste man est que 9a3ed ye5ou fil ism illi taatihoulou ou non
    const {first_name,last_name,occupation,gender,email,phone,birthday,doctor,cin} = req.body
    console.log(req.body)
    if (!last_name || !email || !password || !first_name || !occupation || !gender || !birthday  || !phone || !doctor || !cin )
    {
        return res.status(422).json({error : "please add all fields"})
    }
    //res.json({message:"successfully posted"}) --> njarbou beha fil postman 
    User.findOne({cin : email})
    .then((savedUser)=>{
        if(savedUser){
            return res.status(422).json({error : "user already exists with this email"})
        }
