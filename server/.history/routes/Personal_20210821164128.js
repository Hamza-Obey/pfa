...
router.get('/Patients',requireLoginUser,(req,res)=>{
    ...
    .then(result=>{
        for (let i = 0; i < result.length; i++) {
            data.push({ id: result[i]._id, first_name: result[i].first_name, email: result.client[i].email, cin: result[i].cin, imgUrl: result.client[i].pic, tel: result.client[i].tel, status: result.client[i].status, age: result.client[i].age })
        }
        res.json(data)
    })
    .then ((err)=>{
