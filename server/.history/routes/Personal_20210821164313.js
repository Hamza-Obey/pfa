...
router.get('/Patients',requireLoginUser,(req,res)=>{
    ...
    .then(result=>{
        for (let i = 0; i < result.length; i++) {
            data.push({ id: result[i]._id, first_name: result[i].first_name, email: result[i].email, cin: result[i].cin, phone: result[i].phone, state: result[i].state, birthday: result[i].birthday,gender:result[i].gender,occupation:result[i] })
        }
        res.json(data)
    })
    .then ((err)=>{
